...
const MDM_TRANSLATIONS = {
  ...
  en: {
    ...
    ,"根据客户的设备范围和数据交换要求，决定任务发送、事件通知及扩展资料的处理方式。": "Determine task delivery, event notifications, and extension-data handling based on the customer's equipment scope and data-exchange requirements.", "设备执行": "Equipment execution", "指定类型的仓储任务由自动化设备完成": "Have automated equipment complete selected warehouse task types", "结果通知": "Result notifications", "客户系统需要及时接收关键业务结果": "Ensure the customer system receives key business results promptly", "客户化资料": "Customer-specific data", "标准字段不足以承载客户自己的参考信息": "Standard fields cannot hold the customer's own reference information"
  }
};
