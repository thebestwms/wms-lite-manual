...
const MDM_TRANSLATIONS = {
  ja: {
    ...
    ,"换算关系": "換算関係", "当前要建立的单位": "登録する単位", "包含的下一级单位数量": "下位単位の数量", "内部使用的下一级单位": "内部で使用する下位単位", "库存统一换算、比较和分配时使用的基础单位，通常是最小可管理单位。": "在庫の換算、比較、引当に使用する基準単位で、通常は最小管理単位です。", "新建部分业务明细时优先带出的单位，可以与 Base UOM 不同。": "一部の業務明細作成時に優先される単位で、Base UOM と異なる場合があります。", "包装补充资料": "包装補足情報", "多级包装换算关系": "多段階包装の換算関係", "系统换算结果": "システム換算結果", "维护限制": "管理上の制限", "已有库存使用的 UOM 不能修改名称、换算数量或删除。": "既存在庫で使用中の UOM は名称、換算数量の変更や削除ができません。", "记录商品在托盘上的标准码放结构": "パレット上の商品標準積付け構造を記録する", "动态字段": "動的フィールド"
  en: {
